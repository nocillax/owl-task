...
export default function WhyChooseUs() {
  return (
    <section id="about" className="px-6 py-24 mb-10 w-full bg-white">
      <div className="max-w-6xl mx-auto bg-[#FDF4F6] rounded-[2rem] lg:rounded-[3rem] overflow-hidden relative">
        <div className="flex flex-col lg:flex-row items-center lg:items-stretch justify-between z-10 relative">
          ...
          <div className="order-3 lg:order-none flex-1 flex flex-col justify-center items-center sm:items-start p-8 md:p-12 lg:p-[4.5rem] lg:pr-8 z-10 w-full">
            <h2 className="hidden lg:block text-3xl md:text-5xl font-extrabold mb-8 text-[#0f0f0f] tracking-tight leading-tight">
              Few Reasons Why you
              <br />
              Choose us?
            </h2>
            <p className="text-[#3a3a3a] text-sm font-medium mb-12 leading-relaxed max-w-md text-center sm:text-left">
            ...
            <ul className="flex flex-col gap-[1.125rem] mb-[3.25rem] w-full sm:w-auto">
              {REASONS.map((reason, idx) => (
                <li
                  key={idx}
                  className="flex items-center gap-4 text-[#1f1f1f] font-bold text-sm"
                >
                  <div className="flex items-center justify-center w-5 h-5 rounded-full bg-[#FFD6E0] text-primary shrink-0 transition-transform hover:scale-110">
            ...
            <button className="flex items-center gap-2 bg-primary text-white border-none rounded px-8 py-3.5 font-bold text-sm hover:opacity-90 transition-opacity shadow-sm tracking-wide">

...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="px-6 py-8 bg-white w-full">
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        ...
        <div className="flex flex-col items-center mb-8 text-center text-[#1f1f1f]">
          ...
          <p className="text-[#191D23] text-xl font-regular">
            Choose a plan that's right for you
          </p>
        </div>

        {/* Toggle Controls */}
        <div className="flex justify-center items-center gap-5 mb-20 relative">
          <span
            className={`text-sm font-semibold transition-colors ${!isYearly ? "text-[#191D23]" : "text-[#888888]"}`}
          >
            Pay Monthly
          </span>
          <button
          ...
          >
            <div
              className={`w-4 h-4 bg-white rounded-full transition-transform shadow-sm ${isYearly ? "translate-x-5" : "translate-x-0"}`}
            />
          </button>
          <span
            className={`text-sm font-semibold transition-colors ${isYearly ? "text-[#191D23]" : "text-[#888888]"}`}
          ...
          <div className="absolute left-1/2 ml-20 md:ml-28 top-1/2 -translate-y-1/2 flex items-center z-0">
            <img
              src="https://placehold.co/60x30/transparent/e33b65?text=~>"
              className="w-12 object-contain opacity-90 -rotate-12"
              alt="Arrow pointing to pay yearly"
            />
            <span className="text-primary font-extrabold text-sm ml-1 absolute left-[85%] whitespace-nowrap">
              Save 25%
            </span>
          </div>
        </div>
